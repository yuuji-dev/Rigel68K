...
char endIncludeString[] = "-------------------- end include --------------------\n";
...
int opt(int size, char *label, char *op, int *errorPtr)
{
  ...
  while (isalnum(*op) && !done)
  {
    ...
    if ( strcasecmp(option,"CRE") == 0)              // if CRE option
      CREflag = true;             // enable CRE (display symbol table in listing)
    else if ( strcasecmp(option,"MEX") == 0)
      MEXflag = true;             // enable macro expansion
    else if ( strcasecmp(option,"NOMEX") == 0)
      MEXflag = false;            // disable macro expansion
    else if ( strcasecmp(option,"SEX") == 0)
      SEXflag = true;             // enable structured expansion
    else if ( strcasecmp(option,"NOSEX") == 0)
      SEXflag = false;            // disable structured expansion
    else if ( strcasecmp(option,"WAR") == 0)
      WARflag = true;             // show warnings
    else if ( strcasecmp(option,"NOWAR") == 0)
      WARflag = false;            // suppress warnings
    else if ( strcasecmp(option,"CEX") == 0)
      CEXflag = true;             // enable constant expansion
    else if ( strcasecmp(option,"NOCEX") == 0)
      CEXflag = false;            // disable constant expansion
    else if ( strcasecmp(option,"BIT") == 0) {
      BITflag = true;             // enable bit field support
      if (pass2 && listFlag) {
        char temptext[] = "*[sim68k]bitfield\n";
        listLine(temptext, "\0");        // enables bit field support in Sim68K
      }
    }
    else
      NEWERROR(*errorPtr, SYNTAX);
...
int memory(int size, char *label, char *op, int *errorPtr)
{
  ...
  if ( strcasecmp(operand,"ROM") == 0)            // if ROM map
    ROMmap = true;
  else if ( strcasecmp(operand,"READ") == 0)      // if Read map
    readMap = true;
  else if ( strcasecmp(operand,"PROTECTED") == 0) // if Protected map
    protectedMap = true;
  else if ( strcasecmp(operand,"INVALID") == 0)   // if Invalid map
    invalidMap = true;
  else {
    NEWERROR(*errorPtr, SYNTAX);
    return NORMAL;
