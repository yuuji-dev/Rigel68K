...
int writeObj()
{
  ...
  try {
    ...
    for (int i=0; i<byteCount; i++) {
      ...
      str += *sRec++;
      str += *sRec++;
      checksum += (char) ((std::stoi(str) & 0xFF) & 0xFF);
    }
...
int finishObj()
{
  ...
  try {
    ...
    if (mapROM) {
      //                     R O M
      char temptext[] = "202020202020524F4D20";
      writeMap(temptext, mapROMStart, mapROMEnd);
    }
    if (mapRead) {
      //                   R E A D
      char temptext[] = "20202020205245414420";
    
      writeMap(temptext, mapReadStart, mapReadEnd);
    }
    if (mapProtected) {
      //         P R O T E C T E D
      char temptext[] = "50524F54454354454420";
      writeMap(temptext, mapProtectedStart, mapProtectedEnd);
    }
    if (mapInvalid) {
      //             I N V A L I D
      char temptext[] = "2020494E56414C494420";

      writeMap(temptext, mapInvalidStart, mapInvalidEnd);
    }
